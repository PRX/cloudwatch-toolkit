...
function conditionAlarmTime(event) {
  if (event?.detail?.previousState?.reasonData) {
    ...
    if (previousData?.evaluatedDatapoints?.length) {
...
function duration(event) {
  ...
  if (okTime && alarmTime) {
    ...
    let durationValue = difSec;
    let durationUnit = "seconds";

    if (difSec >= 3600) {
      durationValue = Math.round(difSec / 3600);
      durationUnit = "hours";
    } else if (difSec >= 60) {
      durationValue = Math.round(difSec / 60);
      durationUnit = "minutes";
    }

    return `*Alarm duration:* ${durationValue} ${durationUnit} | `;
  }
